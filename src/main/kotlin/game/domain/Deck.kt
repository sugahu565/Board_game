...
class Deck(private val playerCount: Int) {
    ...
    private fun buildAndShuffleDeck() {
        ...
        repeat(6) { safeCards.add(Villager) }
        ...
        val hunter = safeCards.first { it.role == CardRole.HUNTER }
        safeCards.remove(hunter)
        initialDeal.add(hunter)

        val hide = safeCards.first { it.role == CardRole.HIDE }
